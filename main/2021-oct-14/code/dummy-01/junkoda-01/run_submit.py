...
def run_submit(
    ...
    fold = 1
    out_dir = root_dir + '/result/run02/junkoda-01/fold%d' % fold
    initial_checkpoint = \
        out_dir + '/checkpoint/00100000.model.pth'  #None #
    ...
    scaler = read_pickle_from_file(out_dir+'/scaler.pickle')
    ...
    if 1:
        ...
        submit_df.to_csv(submit_dir+'/df_submit.csv', index=False)
        np.save(submit_dir+'/valid_idx.npy', valid_idx)
...
def run_show_results():
    df = make_df(mode='train')
    submit_dir = '/root/share1/kaggle/2021/ventilator/result/run01/lstm-learnable-split2-32-f8/fold1/valid/submit_local_00100000.model'
